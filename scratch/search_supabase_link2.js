...
async function run() {
  ...
  console.log("Searching for supabase link step details (part 2)...");
  
  for await (const line of rl) {
    if (!line.trim()) continue;
    try {
      const step = JSON.parse(line);
      if (step.step_index >= 2670 && step.step_index <= 3800) {
        const contentStr = JSON.stringify(step);
        if (contentStr.toLowerCase().includes('token') || contentStr.toLowerCase().includes('supabase link')) {
